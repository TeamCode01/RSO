...
def calculate_r14():
    ...
    logger.info('Выполняется подсчет отчета по 14 показателю')
    ...
    if not ro_ids_without_14_reports:
        logger.info('Нет региональных штабов, у которых нет отчета по 14 показателю')
        return
    ...
    ro_reports = RegionalHeadquarter.objects.filter(
        ...
        'id', 'regionalr12_id', 'regionalr13_id', 'regionalr12__amount_of_money', 'regionalr13__number_of_members'
    )
    ...
    for ro in ro_reports:
        reports_to_create.append(RegionalR14(
            ...
            report_12_id=ro['regionalr12_id'],
            report_13_id=ro['regionalr13_id'],
            score=ro['regionalr13__number_of_members'] / ro['regionalr12__amount_of_money'],
        ))

    logger.info(f'Создаем {len(reports_to_create)} отчетов по 14 показателю')
    try:
        new_reports = RegionalR14.objects.bulk_create(reports_to_create)
    except Exception as e:
        logger.exception(f'Не удалось создать отчеты по 14 показателю: {e}')

    logger.info(f'Завершено подсчет отчета по 14 показателю. Создано {len(new_reports)} отчетов')
